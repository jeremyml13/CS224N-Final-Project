...
def model_eval_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        para_false_pos_rate = np.mean(np.logical_and(np.array(para_y_pred) == 1, np.array(para_y_true) == 0))
        para_false_neg_rate = np.mean(np.logical_and(np.array(para_y_pred) == 0, np.array(para_y_true) == 1))
        para_true_neg_rate = np.mean(np.logical_and(np.array(para_y_pred) == 0, np.array(para_y_true) == 0))
        para_true_pos_rate = np.mean(np.logical_and(np.array(para_y_pred) == 1, np.array(para_y_true) == 1))
        ...
        print(f'Paraphrase detection false positive rate: {para_false_pos_rate:.3f}')
        print(f'Paraphrase detection false negative rate: {para_false_neg_rate:.3f}')
        print(f'Paraphrase detection true negative rate: {para_true_neg_rate:.3f}')
        print(f'Paraphrase detection true positive rate: {para_true_pos_rate:.3f}')
